...
function control_Loading_start(loading){
	...
	var ajaxCallback = control_Loading_finish.bind(null,loading);
	control_ajax_request(request,ajaxCallback);
	...
	for(i in hiddenObject){
		...
		if(typeof(object) == 'object'){
			if(object instanceof HTMLElement)
				command_Loading_hide(object);
		}
	}
	if(typeof(callback) == 'object'){
		if(callback instanceof HTMLElement)
			command_Loading_show(callback);
	}
...
function control_Loading_finish(loading, data){
	...
	if(typeof(dataObject) == 'object'){
		if(dataObject instanceof HTMLImageElement)
			command_Loading_refresh(dataObject);
